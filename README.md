...
Most types say how to call a tool or make a web request. A few say only which of TeaNode's own readers to use, and with what settings: reading a folder of files and git checkouts (`folder`), a folder of dated notes (`journal`), or a website (`website`) is code built into TeaNode, not a list of calls. They are types here all the same, so that every source a person has is of a type from this registry, and adding any of them is the same form.
...
A type either names a `reader` built into TeaNode (`files`, `journal`, `web`) and declares the settings it takes, and is then complete, or says how to list and read a source with commands and requests, as below.
...
`runs` says where a type can run: on one of the person's attached computers (`computer`, the only choice for a type that runs commands), on the TeaNode server itself (`server`), or either, chosen per source; left out, it is `computer`. A type that only makes web requests can run on the server; one whose service answers only inside a network runs through a computer in it.

Anywhere a `command` can be given, a `request` can be given instead: a web request with `method`, `url`, `headers`, `body` and `auth`, as in a skill. A credential comes from `secrets`, each declared with its `key`, a `description` and a `scope` (`operator`, one value for everybody, or `person`), kept encrypted on the server and referred to as `{{secret:key}}`, and used through `authenticationProfiles` (`bearer`, `basic`, `apiKey`). The rule skills keep holds here too: a credential is sent only to a host written into the type or taken from a secret, except that a person's own secret may go to an address that person gave in the source's settings; never an operator's.
...
`records` is how to read one container: a list of readings, each a `command`, `parse`, `paging`, and a `record` block giving each field of the record (`id`, `kind`, `title`, `url`, `at`, `modifiedAt`, `author`, `channel`, `thread`, `private`, `text`, `version`) from the item. An `author` of `@you` is the person whose computer it is, for a type that knows a post is theirs but not their name; TeaNode files it under their username. A reading can run once `each` value of a list written into it, `skip` items a condition picks out, and say that a command's not-found answer means `missing: empty` rather than a failure.

A listing can also be `fixed`: containers written into the type rather than listed, for a source that is one file (a mailbox).

`detail`, inside a reading, is a command run for an item whose `version` changed since the last pass, supplying its text. `attachments` is a command, or a list of them each with a `when`, that writes one file of an item to the path it is given as `{{output}}`, up to `maxBytes`. An attachment names its files with `each` (a list the item already has) or with `list` (a command of its own, parsed like any other, whose items are the files, asked once for each version of the item); where the tool gives a file a new identifier each time it is asked, `key` names it instead, so it is not fetched again. Both are cached on the computer by item and version, so an unchanged item costs nothing.
...
Templates: `{{settings.x}}`, `{{container.x}}`, `{{item.x}}`, `{{each}}`, `{{parent.x}}` for the container a listing runs `over`, `{{folder.x}}` for the folder a `walk` is in, `{{response.x}}` for the rest of the answer an item came in (with `[...]` to look a value up by another), `{{detail.text}}`, `{{output}}`, and `{{pass.since}}`, `{{pass.windowStart}}`, `{{pass.windowEnd}}`. Filters: `epoch-ms` (milliseconds as a time), `epoch` (a time as seconds since 1970), `date`, `join ", "`, `replace "/" " "`, `urlencode`, `html-text` (HTML as plain text), `or <fallback>`, `empty`, and `flag "<when true>" "<when false>"`. A condition (`skip`, `when`, `branch`, `unchangedWhen`) compares with `==`, `!=`, `<=`, `in [...]` and `matches <pattern>`, joined with `&&`, `||` and `!`.

`parse` is `json` with `items` (a dotted path to the list; a path ending in `.*` takes the values of a map, and `a | b` tries each path in turn), `xml` with `items` the same way, `jsonl` (one object a line), `lines` (a regular expression whose named groups become the item's fields, with lines that do not match skipped), or `text` (the whole output, as `text`).
